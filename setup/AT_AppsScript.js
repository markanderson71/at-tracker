...
function handleRequest(e) {
  try {
    ...
    // Also check POST body for action/sheet (used when everything goes through POST to avoid CORS)
    var postBody = null;
    if (e.postData && e.postData.contents) {
      try { postBody = JSON.parse(e.postData.contents); } catch(pe) {}
    }
    if (!action && postBody && postBody._action) action = postBody._action;
    if (!sheet && postBody && postBody._sheet) sheet = postBody._sheet;
    ...
    switch (action) {
      ...
      case "create":
        var createData = postBody || (e.postData ? JSON.parse(e.postData.contents) : {});
        result = createRow(sheet, createData);
        break;

      // ── UPDATE: Update a row by id ───────────────────────
      case "update":
        var updateData = postBody || (e.postData ? JSON.parse(e.postData.contents) : {});
        result = updateRow(sheet, updateData);
        break;

      // ── DELETE: Delete a row by id ───────────────────────
      case "delete":
        var deleteId = (e.parameter && e.parameter.id) || (postBody && postBody._id) || "";
        result = deleteRow(sheet, deleteId);
        break;

      // ── BATCH UPDATE: Update multiple rows at once ───────
      case "batchUpdate":
        var batchData = postBody || (e.postData ? JSON.parse(e.postData.contents) : {});
    ...
    var callback2 = (e.parameter && e.parameter.callback) || "";
...
function addDiaryColumns() {
  ...
  var headers = ws.getRange(1, 1, 1, ws.getLastColumn()).getValues()[0];
  var newCols = ["attachments", "comments", "readBy"];
